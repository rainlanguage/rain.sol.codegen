...
/// @title IOpcodeToolingV1NoticeTest
/// @notice `IOpcodeToolingV1` is published and consumed as an ABI artifact, so
/// its contract level documentation is only as good as the `userdoc` a
...
contract IOpcodeToolingV1NoticeTest is Test {
    /// The interface whose emitted NatSpec is asserted here.
    string internal constant CONTRACT_NAME = "IOpcodeToolingV1";

    /// The source file the compiler wrote that interface's artifact for.
    string internal constant SOURCE_FILE_NAME = "IOpcodeToolingV1.sol";
