...
contract LibCodeGenFilePrefixTest is Test {
    ...
    /// The two exported defaults are this repo's own licence and copyright,
    /// pinned as literals rather than read back out of the header they build.
    /// They are what every Rain repo's generated files declare, so a change to
    /// either rewrites committed source org wide and has to land as a
    /// deliberate diff here.
    function testRainDefaultsExact() external pure {
        assertEq(RAIN_SPDX_LICENSE_IDENTIFIER, "LicenseRef-DCL-1.0");
        assertEq(RAIN_COPYRIGHT_TEXT, "Copyright (c) 2020 Rain Open Source Software Ltd");
    }

    /// A Rain repo gets Rain's licence and copyright without passing them, and
    /// what it gets is byte for byte what a caller naming the two exported
    /// defaults gets. The short overload is the long one applied to those two
    /// constants and nothing else, so there is no second spelling of the header
    /// that can drift from the first.
    function testFilePrefixDefaultEqualsExplicitRainValues() external pure {
        assertEq(LibCodeGen.filePrefix(), LibCodeGen.filePrefix(RAIN_SPDX_LICENSE_IDENTIFIER, RAIN_COPYRIGHT_TEXT));
    }

    /// The defaulted header, pinned to the literal. The equality above is
    /// satisfied by both sides being wrong in the same way; this is what says
    /// which bytes they both have to be.
    function testFilePrefixDefaultExact() external pure {
        assertEq(
    ...
    /// The defaults are values the long overload accepts, so the short one is
    /// reachable rather than a call that always reverts. Nothing about the two
    /// constants is exempt from the rule the header puts on any other value.
    function testFilePrefixDefaultsAreAcceptedValues() external pure {
        assertTrue(LibCodeGen.isSingleLine(RAIN_SPDX_LICENSE_IDENTIFIER), "default licence is not a single line");
        assertTrue(LibCodeGen.isSingleLine(RAIN_COPYRIGHT_TEXT), "default copyright is not a single line");
    }
}
//REUSE-IgnoreEnd
